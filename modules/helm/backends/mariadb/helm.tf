...
resource "kubernetes_secret" "auth" {
  ...
  data = {
    "${local.mariadb_secret_root_password_key}" = var.database_root_password
    "${local.mariadb_secret_user_password_key}" = var.database_password
    "${local.mariadb_secret_replication_password_key}" = var.database_replication_password
  }
