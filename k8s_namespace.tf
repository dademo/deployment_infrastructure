resource "kubernetes_namespace" "namespace" {
    ...
    for_each = toset(concat(
        var.module_istio.enabled ? ["istio"] : [],
        var.module_supervision.enabled ? ["supervision"] : [],
        var.module_keycloak.enabled ? ["keycloak"] : [],
        var.module_gitea.enabled ? ["gitea"] : [],
        var.module_jenkins.enabled ? ["jenkins"] : [],
        var.module_gocd.enabled ? ["gocd"] : [],
        var.module_nexus.enabled ? ["nexus"] : [],
        var.module_sonarqube.enabled ? ["sonarqube"] : [],
        var.module_minio.enabled ? ["minio"] : [],
        var.module_dokuwiki.enabled ? ["dokuwiki"] : [],
        var.module_nextcloud.enabled ? ["nextcloud"] : [],
        var.module_dev.enabled ? ["dev"] : [],
    ))
